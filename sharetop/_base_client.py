...
DEFAULT_BASE_URL = "https://api.sharetop.top/api"
...
class BaseClient:
    ...
    def __init__(
        ...
        if api_key is None or api_key == "":
            self.api_key = os.environ.get("SHARETOP_API_KEY")
        else:
            self.api_key = api_key

        # Only raise error if we're trying to use the default paid URL without a key
        if not self.api_key:
            effective_base_url = base_url or os.environ.get("SHARETOP_BASE_URL")
            if effective_base_url is None or effective_base_url == DEFAULT_BASE_URL:
                raise ValueError(
                    "Token is required for paid API. "
                    "无效Token，请关注公众号：“浔溯de小仓鼠”进行获取"
                )
        ...
        self.base_url = (
            base_url or os.environ.get("SHARETOP_BASE_URL") or DEFAULT_BASE_URL
        ).rstrip("/")
    ...
    def _build_headers(self, extra_headers: Optional[Headers] = None) -> dict[str, str]:
        ...
        headers = {
            ...
            "User-Agent": f"sharetop-python-sdk/{__version__}",
            **self._default_headers,
        }
        # Only add API key if present (free tier doesn't need it)
        if self.api_key:
            headers["Authorization"] = ' '.join(["Bearer", self.api_key])
        if extra_headers:
...
class SyncAPIClient(BaseClient):
    """Synchronous HTTP client for ShareTop API with automatic retry.
